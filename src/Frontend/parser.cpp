...
ExprPtr Parser::parseLet() {
  ...
  Common::Span letSpan = letTok.span;

  if (!check(TokenType::Identifier)) {
    emitError(
        "expected identifier after 'let'",
        std::string("write: let <name> = <expr>  or  let <name>(...) { ... }"),
        peek().span, Common::ParserError::ExpectedIdentifierAfterLet);
    return nullptr;
  }
  ...
  if (match(TokenType::Delimiter, "(")) {
    ...
    Common::Span paramStart = previous().span;
    ...
    while (!check(TokenType::Delimiter, ")") && !atEnd()) {
      if (!check(TokenType::Identifier)) {
        emitError("expected parameter name",
                  ...
                  peek().span, Common::ParserError::ExpectedParameterName);
        synchronize();
        return nullptr;
      }
    ...
    expect(TokenType::Delimiter, ")", "expected ')' after parameter list",
           Common::ParserError::ExpectedClosingParen);

    if (!check(TokenType::Delimiter, "{")) {
      emitError("expected '{' after parameter list",
                std::string("function body must be a block: let " + name +
                            "(...) { ... }"),
                peek().span, Common::ParserError::ExpectedBlockAfterParams);
      return nullptr;
    }

    Block body = parseBlock();
    Common::Span lambdaSpan = Common::Span::merge(paramStart, body.span);
  ...
  expect(TokenType::Operator, "=", "expected '=' after name in let binding",
         Common::ParserError::ExpectedEquals);
...
ExprPtr Parser::parseIf() {
  ...
  if (!cond) {
    emitError("expected condition after 'if'", std::nullopt, peek().span,
              Common::ParserError::ExpectedIfCondition);
    return nullptr;
  }
  ...
  if (!check(TokenType::Delimiter, "{")) {
    emitError("expected '{' after if condition",
              ...
              peek().span, Common::ParserError::ExpectedThenBlock);
    return nullptr;
  }
  ...
  if (match(TokenType::Keyword, "else")) {
    if (!check(TokenType::Delimiter, "{")) {
      emitError("expected '{' after 'else'",
                ...
                peek().span, Common::ParserError::ExpectedElseBlock);
      return nullptr;
    }
    elsBlock = parseBlock();
  }

  Common::Span fullSpan = ifTok.span;
  if (elsBlock)
    fullSpan = Common::Span::merge(fullSpan, elsBlock->span);
  else
    fullSpan = Common::Span::merge(fullSpan, thenBlock.span);
  ...
  while (true) {
    ...
    if (!rhs) {
      emitError("expected expression after '" + std::string(op.value) + "'",
                ...
                Common::ParserError::ExpectedRhsAfterBinaryOp);
      return nullptr;
    }

    Common::Span span = Common::Span::merge(spanOf(*lhs), spanOf(*rhs));
    lhs = makeExpr(Binary{
        .lhs = std::move(lhs),
  ...
  if (check(TokenType::Operator, "-") || check(TokenType::Operator, "!")) {
    ...
    if (!rhs) {
      emitError("expected expression after unary '" + opTok.value + "'",
                std::nullopt, peek().span,
                Common::ParserError::ExpectedRhsAfterUnaryOp);
      return nullptr;
    }

    Common::Span span = Common::Span::merge(opTok.span, spanOf(*rhs));
  ...
  while (true) {
    if (match(TokenType::Delimiter, "(")) {
      ...
      expect(TokenType::Delimiter, ")", "expected ')' after argument list",
             Common::ParserError::ExpectedClosingCallParen);
      Common::Span callSpan =
          Common::Span::merge(spanOf(*expr), previous().span);
      expr = makeExpr(Call{
      ...
      Common::Span faSpan = Common::Span::merge(spanOf(*expr), field.span);
...
ExprPtr Parser::parsePrimary() {
  ...
  if (tok.type == TokenType::Delimiter && tok.value == "(") {
    ...
    expect(TokenType::Delimiter, ")",
           ...
           Common::ParserError::ExpectedClosingGroupParen);
    return inner;
  }
  if (tok.type != TokenType::Eof) {
    emitError(
        ...
        tok.span, Common::ParserError::UnexpectedToken);
    advance();
  }
  return nullptr;
}
...
Block Parser::parseBlock() {
  ...
  Common::Span blockSpan = open.span;
  std::vector<ExprPtr> exprs;
...
ExprPtr Parser::parsePackage() {
  ...
  if (!check(TokenType::Identifier)) {
    emitError("expected package name after 'package'",
              ...
              Common::ParserError::ExpectedIdentifierAfterLet);
    return nullptr;
  }
  ...
  return makeExpr(Package{
      .name = nameTok.value,
      .span = Common::Span::merge(kwTok.span, nameTok.span),
  });
}
...
ExprPtr Parser::parseImport() {
  ...
  return makeExpr(Import{
      ...
      .span = Common::Span::merge(kwTok.span, strTok.span),
  });
}
} // namespace Iris::Frontend