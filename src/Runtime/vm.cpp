...
#define BINARY_NUM_OP(op, opcode_name, op_str)                                 \
  case OpCode::opcode_name: {                                                  \
    ...
    emitError("'" op_str "' requires two numbers", std::nullopt,               \
              Common::RuntimeError::TypeError);                                \
    return std::nullopt;                                                       \
  }

namespace Iris::Runtime {

VM::VM(std::string_view filename, std::string_view source,
...
void VM::emitError(std::string msg, std::optional<std::string> hint,
  ...
  m_bag.emit(Common::Diagnostic{.severity = Common::Severity::Error,
                                .code = code,
                                .hint = std::move(hint),
...
void VM::push(Value v) {
  if (m_stack.size() >= kMaxStack) {
    emitError("stack overflow", std::nullopt,
              Common::RuntimeError::StackOverflow);
    return;
  }
...
bool VM::call(std::shared_ptr<Closure> closure, uint8_t argc) {
  if (argc != closure->arity) {
    emitError("arity mismatch: expected " + std::to_string(closure->arity) +
              ...
              std::nullopt, Common::RuntimeError::ArityMismatch);
    return false;
  }
...
std::optional<Value> VM::execute() {
  while (true) {
    ...
    case OpCode::Call: {
      ...
      if (!cl) {
        emitError("value is not callable", std::nullopt,
                  Common::RuntimeError::NotCallable);
        return std::nullopt;
      }
        ...
        if (auto *b = std::get_if<double>(&rhs)) {
          if (*b == 0.0) {
            emitError("division by zero", std::nullopt,
                      Common::RuntimeError::DivisionByZero);
            return std::nullopt;
          }
          push(*a / *b);
          break;
    ...
    case OpCode::Neg: {
      ...
      emitError("'-' requires a number", std::nullopt,
                Common::RuntimeError::TypeError);
      return std::nullopt;
    }
    case OpCode::Not: {
