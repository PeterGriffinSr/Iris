...
ModuleLoader::loadImports(std::span<const ExprPtr> program,
                          const std::filesystem::path &importingFile) {
  std::vector<LoadedPackage> result;

  for (const ExprPtr &expr : program) {
    const Import *imp = std::get_if<Import>(&expr->val);
    if (!imp)
      continue;
...
ModuleLoader::compilePackageFile(const std::filesystem::path &filePath,
  ...
  DiagnosticBag fileBag(m_opts);
  std::string pathStr = filePath.string();

  Lexer lexer(source, pathStr, fileBag);
  ...
  Parser parser(tokens, pathStr, source, fileBag);
  ...
  Resolver resolver(pathStr, source, fileBag);
